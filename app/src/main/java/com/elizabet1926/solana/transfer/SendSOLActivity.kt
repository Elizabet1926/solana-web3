...
private fun SendSOLScreen(solanaWeb: SolanaWeb) {
    ...
    Column(
        ...
        OutlinedTextField(value = secretKey, onValueChange = { secretKey = it }, modifier = Modifier.fillMaxWidth(), minLines = 2, placeholder = { Text("Enter sender private key") })
        Text("From Address (optional)", style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
        OutlinedTextField(value = fromAddress, onValueChange = { fromAddress = it }, modifier = Modifier.fillMaxWidth(), placeholder = { Text("Leave empty to use address from private key") })
        Text("To Address", style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
        OutlinedTextField(value = toAddress, onValueChange = { toAddress = it }, modifier = Modifier.fillMaxWidth(), placeholder = { Text("Recipient address (base58)") })
        Text("Amount (SOL)", style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
        OutlinedTextField(value = amount, onValueChange = { amount = it }, modifier = Modifier.fillMaxWidth(), placeholder = { Text("0.1") })
        Text("Estimated Cost", style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
        OutlinedButton(
            onClick = {
                val to = toAddress.trim().replace("\n", "")
                val amt = amount.trim().toDoubleOrNull()
                if (fromAddress.trim().isEmpty() && secretKey.trim().isEmpty()) {
                    estimateText = "Please enter private key or from address."
                    return@OutlinedButton
                }
                if (to.isEmpty()) { estimateText = "Please enter recipient address."; return@OutlinedButton }
                if (amt == null || amt <= 0) { estimateText = "Please enter a valid amount (SOL)."; return@OutlinedButton }
                val endpoint: String = RPCConfigManager.currentEndpoint ?: run { estimateText = "No RPC endpoint."; return@OutlinedButton }
                val lamports = (amt * LAMPORTS_PER_SOL).toLong()
                ...
                scope.launch {
                    ...
                    if (fromPub.isEmpty()) {
                        ...
                        if (fromPub.isEmpty()) {
                            estimateText = (r?.get("error") as? String) ?: "Failed to get sender address."
                            return@launch
                        }
                    }
                    val r = solanaWeb.estimatedSOLTransferCostAsync(endpoint, fromPub, to, lamports)
                    val err = r?.get("error") as? String
                    if (err != null) { estimateText = err; return@launch }
                    val lines = mutableListOf<String>()
                    r?.get("transferSol")?.let { lines.add("Transfer: $it SOL") }
                    r?.get("estimatedFeeSol")?.let { lines.add("Estimated fee: $it SOL") }
                    r?.get("estimatedSOLTransferCost")?.let { lines.add("Total (transfer + fee): $it SOL") }
                    estimateText = lines.joinToString("\n").ifEmpty { "No estimate data." }
                }
            },
        ...
        Button(
            onClick = {
                ...
                if (to.isEmpty()) { resultText = "Please enter recipient address."; isError = true; return@Button }
                if (amt == null || amt <= 0) { resultText = "Please enter a valid amount (SOL)."; isError = true; return@Button }
                ...
                loading = true
                scope.launch {
                    val r = solanaWeb.solanaMainTransferAsync(endpoint, sk, to, lamports)
                    loading = false
                    ...
                    if (sig != null) {
                        ...
                        resultText = (r?.get("error") as? String) ?: "Failed to send SOL"
        ...
        ) { Text(if (loading) "Sending…" else "Send SOL") }
