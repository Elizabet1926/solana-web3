...
private fun MultisigApproveScreen(solanaWeb: SolanaWeb) {
    ...
    Scaffold(
        ...
        Button(
            onClick = {
                ...
                loading = true
                scope.launch {
                    val r = solanaWeb.proposalApproveAsync(ep, pk, multi, txIndex)
                    loading = false
                    val sig = r?.get("signature") as? String
                    val err = r?.get("error") as? String
                    if (sig != null) {
                        resultText = "Proposal approved.\nSignature: $sig"
                        lastJson = JSONObject().apply { put("signature", sig) }.toString(2)
                        isError = false
                    } else {
                        resultText = err ?: "Failed to approve proposal"
        ...
        ) { Text(if (loading) "Approving…" else "Approve Proposal") }
