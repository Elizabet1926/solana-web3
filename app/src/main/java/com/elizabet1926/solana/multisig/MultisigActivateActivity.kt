...
private fun MultisigActivateScreen(solanaWeb: SolanaWeb) {
    ...
    Scaffold(
        ...
        Button(
            onClick = {
                ...
                loading = true
                scope.launch {
                    val r = solanaWeb.proposalActivateAsync(ep, pk, multi, txIndex)
                    loading = false
                    val sig = r?.get("signature") as? String
                    val err = r?.get("error") as? String
                    if (sig != null) {
                        resultText = "Proposal activated.\nSignature: $sig"
                        lastJson = JSONObject().apply { put("signature", sig) }.toString(2)
                        isError = false
                    } else {
                        resultText = err ?: "Failed to activate proposal"
        ...
        ) { Text(if (loading) "Activating…" else "Activate Proposal") }
