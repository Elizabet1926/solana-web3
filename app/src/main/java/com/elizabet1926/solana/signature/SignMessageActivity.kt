...
class SignMessageActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent { SolanaTheme { SignMessageScreen(solanaWeb!!) } }
    }
...
private fun SignMessageScreen(solanaWeb: SolanaWeb) {
    val context = LocalContext.current
    val scope = rememberCoroutineScope()
    var setupDone by remember { mutableStateOf(false) }
    var privateKey by remember { mutableStateOf("") }
    var message by remember { mutableStateOf("") }
    var encodingIndex by remember { mutableStateOf(2) } // 0=utf8, 1=base58, 2=auto
    ...
    Column(
        ...
        OutlinedTextField(value = privateKey, onValueChange = { privateKey = it }, modifier = Modifier.fillMaxWidth(), minLines = 2, placeholder = { Text("Enter private key (base58)") })
        Text("Message", style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
        OutlinedTextField(value = message, onValueChange = { message = it }, modifier = Modifier.fillMaxWidth(), minLines = 3, placeholder = { Text("Enter message to sign (UTF-8 or Base58)") })
        ...
        Button(
            onClick = {
                ...
                val msg = message
                if (pk.isEmpty()) { resultText = "Please enter private key."; isError = true; return@Button }
                if (msg.isEmpty()) { resultText = "Please enter message."; isError = true; return@Button }
                resultText = null
                lastJson = null
                loading = true
                scope.launch {
                    val r = solanaWeb.signMessageAsync(pk, msg, encodingParam)
                    loading = false
                    val sig = r?.get("signature") as? String
                    val pub = r?.get("publicKey") as? String
                    if (sig != null && pub != null) {
                        resultText = "Signature:\n$sig\n\nPublic Key:\n$pub"
                        lastJson = JSONObject().apply { put("signature", sig); put("publicKey", pub) }.toString(2)
                        isError = false
                    } else {
                        resultText = (r?.get("error") as? String) ?: "Failed to sign message"
